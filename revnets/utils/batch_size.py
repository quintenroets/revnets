...
if typing.TYPE_CHECKING:
    from MTL_labels.multitask.model import MultitaskModel  # noqa: autoimport


class TuneModel(pl.LightningModule):
    def __init__(self, model):
        super().__init__()
        self.model: MultitaskModel = model
        self.batch_size = 1
        self.old_batch_size = self.model.multitask.data.batch_size
        self.automatic_optimization = False

    @property
    def data(self):
        return self.model.multitask.data
    ...
    def training_step(self, batch, batch_idx):
        # check if accumulation fits in memory as well
        accumulate_interval = 2 if self.batch_size < self.old_batch_size else 1
        self.model.multitask.combiner.accumulate_grad_batches = accumulate_interval
        for _ in range(accumulate_interval):
...
@cache
def get_max_batch_size(model: MultitaskModel, method="validate"):
    tune_model = TuneModel(model)
    trainer = pl.Trainer(
        accelerator="auto",
    ...
    scale_batch_size_kwargs = {"init_val": model.config.batch_size}
    print(f"Calculating max {method} batch size")

    model.do_log = False
    old_batch_size = model.multitask.data.batch_size
    try:
        trainer.tune(
            tune_model,
            method=method,  # noqa
    ...
    model.do_log = True
    model.multitask.data.batch_size = old_batch_size

    safety_factor = (
        (4 if model.config.devices > 1 else 2) if method == "validate" else 1
    )
